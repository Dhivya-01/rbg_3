...
export function Proof({ withGlow = true, className }) {
  const [counters, setCounters] = useState({});
  const sectionRef = useRef(null);
  ...
  // Intersection Observer for triggering animation & counters
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
  ...
  return (
    <section ref={sectionRef} className={cn("overflow-hidden py-20", className)}>
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-lg text-muted-foreground max-w-3xl mx-auto leading-relaxed">
            Our platform is engineered for speed, reliability, and enterprise-grade
            scalability — so your critical processes run without compromise.
          </p>
        </motion.div>
          ...
          {stats.map((stat, idx) => {
            ...
            return (
              <motion.div
                key={idx}
                data-index={idx}
                className="stat-item observe-item relative text-center p-8 bg-background rounded-xl border group isolate cursor-pointer hover:shadow-glow"
                custom={idx}
                initial="hidden"
                whileInView="visible"
                viewport={{ once: true }}
                variants={fadeInUp}
              >
                <div className="w-12 h-12 mx-auto mb-4 flex items-center justify-center bg-muted rounded-lg">
          ...
          <div className="relative bg-background rounded-2xl p-8 md:p-12 border isolate">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {benefits.map((benefit, idx) => {
                ...
                const variant = idx % 2 === 0 ? slideRight : slideLeft;
                return (
                  <motion.div
                    key={idx}
                    custom={idx}
                    ...
                    viewport={{ once: true }}
                    variants={variant}
                    className="relative flex items-start gap-4 p-6 bg-muted/50 rounded-xl border group isolate cursor-pointer hover:shadow-glow"
                  >
                    <div className="w-12 h-12 bg-background rounded-lg flex items-center justify-center border flex-shrink-0">
            ...
            >
              <Button size="lg" className="group isolate" asChild>
                <a href="#demo">
                  <span>Request Enterprise Demo</span>
                  <ArrowRight className="w-4 h-4 ml-2 group-hover:translate-x-1 transition-transform duration-200" />
                </a>
